...
boost::lockfree::queue<long int> the_queue{the_queue_size};
std::atomic<long int> the_queue_sum{0};

// DYNAMICALLY SIZE ADJUSTED QUEUE
boost::lockfree::queue<long int> the_recorded_queue{the_queue_size};
std::atomic<long int> the_recorded_queue_sum{0};
...
// ========================================================
  ...
  for (long int i = 1; i <= 30000; ++i) {
	...
	random_int1 = random_integer(-50, +50);
	sleep_usec(10);

	the_queue.push(random_int1);
	the_recorded_queue.push(random_int1);  // KEPT AS RECORD WILL NOT POP OUT.
    ...
    // DTStamp();
    // printf("Running PRODUCER thread-id TID:%ld. Producer loopno = %ld \t random_int1 = %ld \n", syscall(SYS_gettid), i, random_int1);
  }
...
// ========================================================
  ...
  for (long int i = 30001; i <= 50000; ++i) {

	random_int2 = random_integer(1, +30);
	sleep_usec(10);

	the_queue.push(random_int2);
	the_recorded_queue.push(random_int2);  // KEPT AS RECORD WILL NOT POP OUT.
